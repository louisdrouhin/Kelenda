...
router.get('/callback/:provider', async (req, res) => {
  ...
  if (!identity) {
    // Liaison manuelle uniquement : pas de compte auto-créé depuis un callback OAuth.
    return res.status(404).json({
      error: 'Aucun compte lié à ce provider. Connectez-vous puis liez ce provider via POST /auth/link/:provider.',
    });
  }
