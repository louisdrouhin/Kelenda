...
// Un secret dédié par paire appelant→notification-service (doc section 5,
// "Authentification service-à-service"). Paire ajoutée : tracking-service
// (email de confirmation à un tuteur, hors pipeline event/préférences qui
// suppose un user_id Kelenda — un tuteur n'en a pas).
const ALLOWED_CALLERS: Record<string, string | undefined> = {
  'tracking-service': process.env.INTERNAL_SECRET_TRACKING_NOTIFICATION,
};
